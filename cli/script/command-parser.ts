...
const USAGE_PREFIX = "Usage: hotload";
const CODE_PUSH_URL = "https://hotload.maxleap.cn";
...
export function showHelp(showRootDescription?: boolean): void {
    if (!wasHelpShown) {
        if (showRootDescription) {
            console.log("HotLoad is a service that allows you to publish mobile app updates directly to your users' devices.\n");
            updateCheck();
        }
...
var argv = yargs.usage(USAGE_PREFIX + " <command>")
    ...
    .command("app", "View and manage your HotLoad-enabled apps", (yargs: yargs.Argv) => {
    ...
    .command("login", "Authenticate with the HotLoad server in order to begin managing your apps", (yargs: yargs.Argv) => {
        isValidCommandCategory = true;
        isValidCommand = true;
        yargs.usage(USAGE_PREFIX + " login [--accessKey <accessKey>]")
            ...
            .example("login", "Logs in to the HotLoad server")
    ...
    .command("register", "Register a new account with the HotLoad server", (yargs: yargs.Argv) => {
        isValidCommandCategory = true;
        isValidCommand = true;
        yargs.usage(USAGE_PREFIX + " register")
            .demand(/*count*/ 1, /*max*/ 2)  // Require one non-optional and one optional argument.
            .example("register", "Creates a new user account on the HotLoad server")
            .check((argv: any, aliases: { [aliases: string]: string }): any => isValidCommand);  // Report unrecognized, non-hyphenated command category.
