/// <reference path="../../definitions/generated/maxleap-hotload.d.ts" />
...
import { AcquisitionStatus } from "maxleap-hotload/script/acquisition-sdk";
import { AccessKey, AccountManager, App, CollaboratorMap, CollaboratorProperties, Deployment, DeploymentKey, DeploymentMetrics, Package, Permissions, UpdateMetrics } from "maxleap-hotload";

var configFilePath: string = path.join(process.env.LOCALAPPDATA || process.env.HOME, ".hotload.config");
...
export var loginWithAccessToken = (): Promise<void> => {
    if (!connectionInfo) {
        return Q.fcall(() => { throw new Error("You are not currently logged in. Run the 'hotload login' command to authenticate with the Hotload server."); });
    }
    ...
    if (standardLoginConnectionInfo.providerName) {
        accessToken = standardLoginConnectionInfo.accessKeyName;
    } else {
...
function serializeConnectionInfo(serverUrl: string, accessToken: string): void {
    ...
    log("\r\nSuccessfully logged-in. Your session token was written to " + chalk.cyan(configFilePath) + ". You can run the " + chalk.cyan("hotload logout") + " command at any time to delete this file and terminate your session.\r\n");
}
